// #6 - Lib para indexedDB engine. 
indexeddbEngine = function() {
	var database;
	var objectStores;
	return {
		//Metodo para checagem se o browser suporta a API indexedDB
		init : function(successCallback, errorCallback) {
			if (window.indexedDB) {
				...
				request.onerror = function(event) {
					errorCallback('storage_not_initalized', 'It is not possible to initialized indexedDB storage');
				}
			} else {
				errorCallback('storage_api_not_supported', 'The web storage api is not supported');
			}			
		},
		//Metodo para iniciar o object store
		initObjectStore  : function(type, successCallback, errorCallback) {
	    	if (!database) {
				errorCallback('storage_api_not_initialized', 'The indexedDB storage engine has not been initialized');
			}
	    	var exists = false;
	    	//checa se o objeto ja existe no banco
